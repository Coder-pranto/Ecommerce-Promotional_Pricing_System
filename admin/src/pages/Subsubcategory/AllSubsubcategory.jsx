...
const AllSubsubcategory = () => {
  ...
  const handleUpdatedData = (subsubcategory) => {
    setselectedSubsubCategory(subsubcategory);
    setIsModalOpen(true);
  };

  const activeChecker = (subsubcategoryId, currentStatus) => {
    handleUpdateSubsubCategory(subsubcategoryId, { status: !currentStatus });
  };

  const handleUpdateSubsubCategory = async (subsubcategoryId, updatedData) => {
    try {
      const response = await axios.patch(
        `http://localhost:5000/api/v1/category/subsub/${subsubcategoryId}`,
        updatedData,
        {
          headers: {
            Authorization: `${token}`,
    ...
    <div className="flex flex-col m-12 items-center mt-20 overflow-auto ">
      <table className="min-w-50% bg-[#FEF4F2] rounded-xl ">
        <thead className="bg-primary text-white">
          <tr>
            ...
            <th className="py-2 px-4">Subsubsubcategory</th>
        ...
        <tbody className="border-2 border-gray-300">
          {subsubcategories.map((subsubcategory, index) => (
            ...
            >
              <td className="py-2 px-4 font-bold">{subsubcategory.name}</td>
              <td className="py-2 px-4">{subsubcategory.description}</td>

              <td className="py-2 px-4 text-center">
                <select className="ml-2 border border-gray-300 rounded px-2 py-1">
                  <option value="">---</option>
                  {subsubcategory.subsubsubCategory.map((subsubsubcategory) => (
                    <option
              ...
              <td
                className={`py-2 px-4 text-center ${subsubcategory.status ? "text-green-500" : "text-red-500"
                  }`}
              >
              ...
              <td className="py-2 px-4 flex items-center">
                <button
                  ...
                  onClick={() =>
                    activeChecker(subsubcategory._id, subsubcategory.status)
                  }
                >
                ...
                <button
                  className="action-btns"
                  onClick={() => handleUpdatedData(subsubcategory)}
                >
                  <FaRegEdit size="25px" />
                </button>
                <button
      ...
      {isModalOpen && (
        <UpdateModal
          ...
          handleUpdateSubsubCategory={handleUpdateSubsubCategory}
